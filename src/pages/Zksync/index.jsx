...
function Zksync() {
    ...
    const handleOk = async () => {
        try {
            ...
            const index = data.findIndex(
                (item) => item.address === values.address
            );
            if (index !== -1) {
                setData(
                    data.map((item, i) => {
                ...
                getZksEra(values.address).then(
                    ({ balance2, tx2, usdcBalance }) => {
                        updatedData[index] = {
                            ...updatedData[index],
                ...
                getEthBalance(values.address, 'ethereum').then(
                    (eth_balance) => {
                        updatedData[index] = {
                            ...updatedData[index],
                            eth_balance,
    ...
    const isRowSatisfyCondition = (record) => {
        ...
        return Object.keys(conditionKeyMapping).every((conditionKey) => {
            ...
            return (
                Number(record[recordKey]) >=
                Number(zkSyncConfigStore[conditionKey])
            );
        });
    };
            ...
            <Content>
                ...
                >
                    <Form form={walletForm} layout="vertical">
                        <Card
                            title="设置钱包预期标准，若钱包达到设置标准，钱包地址背景会为蓝色，更清晰"
                            bordered={true}
                            style={{ width: '100%' }}
                        >
                            <Row gutter={[16, 16]}>
                                ...
                                <Col span={12}>
                                    <FormItem
                                        name="L1ToL2Tx"
                                        addonBefore="L1->L2跨链Tx ≥ "
                                        addonAfter="个"
                                    />
                                    <FormItem
                                        name="L2ToL1Tx"
                                        addonBefore="L2->L1跨链Tx ≥ "
                                        addonAfter="个"
                                    />
                                    <FormItem
                                        name="L1ToL2ETH"
                                        addonBefore="L1->L2跨链金额 ≥ "
                                        addonAfter="ETH"
                                    />
                                    <FormItem
                                        name="L2ToL1ETH"
                                        addonBefore="L2->L1跨链金额 ≥ "
                                        addonAfter="ETH"
                                    />
                                    <FormItem
                                        name="gasFee"
                                        addonBefore="消耗gasFee"
                                        addonAfter="ETH"
                                    />
                                    <FormItem
                                        name="contractMin"
                            ...
                            return (
                                <>
                                    <Table.Summary.Row>
                                        ...
                                        <Table.Summary.Cell dataIndex={4}>
                                            {ethBalance.toFixed(4)}
                                        </Table.Summary.Cell>
                                        <Table.Summary.Cell dataIndex={5} />
                                        <Table.Summary.Cell dataIndex={6}>
                                            {zks1Balance.toFixed(4)}
                                        </Table.Summary.Cell>
                                        <Table.Summary.Cell dataIndex={7} />
                                        <Table.Summary.Cell dataIndex={8}>
                                            {zks2Balance.toFixed(4)}
                                        </Table.Summary.Cell>
                                        <Table.Summary.Cell dataIndex={9}>
                                            {zks2UsdcBalance.toFixed(2)}
                                        </Table.Summary.Cell>
                                        {emptyCells}
                                        <Table.Summary.Cell dataIndex={21}>
                                            {totalFees.toFixed(4)}
                                        </Table.Summary.Cell>
                                    </Table.Summary.Row>
                                </>
                        ...
                        footer={() => (
                            <Card>
                                ...
                                >
                                    ...
                                    >
                                        {batchloading
                                            ? `添加中 进度:(${batchProgress}/${batchLength})`
                                            : '批量添加地址'}
                                    </Button>
                                    <Button
                                        type="primary"
